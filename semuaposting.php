...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <tr style="background-color: yellow;">
      ...
      <td width=400px > <center><H3>Semua Postingan</H3></center></td>
    </tr>
    <tr>
      ...
      <td width=600px rowspan="2"><center>
        <table border="0">
            <?php
            include 'koneksi.php';
            $sql = mysqli_query($conn, "SELECT * from postingan");
            $no=1;
                  foreach ($sql as $row){
                  echo "  <td>
                  <a href='detail2.php?id=".$row['id']."'><img src='".$row['foto']."' width='140' height='130'></a>
                  </td>";
                  echo "  <td>
                  <a href='detail2.php?id=".$row['id']."'><b>".$row['judul']."</b><br><small><i>".$row['status']."&nbsp".$row['tanggal']."&nbsp @".$row['username']."</i></small><br><br></a>".substr($row['caption'],0,210)."...<a href='detail2.php?id=".$row['id']."'>Lihat Selengkapnya</a><br>
                  </td>";
                  $no++;
                    if ($no>1) {
                      echo "<tr></tr>";
